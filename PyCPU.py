...
class PyCPU:
    ...
    def execute(self):
        ...
        if pyinstruction.mnemonic in self.supported_instructions:
            ...
            if not self.supported_instructions[pyinstruction.mnemonic](pyinstruction):
                print "Execution of the instruction itself failed"
                return False
        else:
    ...
    def ADD(self, instruction):
        ...
        #02 /r ADD r/m8,r8 Add r8 to r/m8
        # pnX: added this opcode, like 00 but with opposite direction
            ...
            if op2.type == pydasm.OPERAND_TYPE_REGISTER:
                ...
                op1value = self.get_register(op2.reg, size)
                op2value = self.get_memory_address(instruction, 1, size)
                ...
                self.set_flags("ADD", op2valuederef, op1value, result, size)
    ...
    def AND(self, instruction):
        ...
        #80 /r AND r/m8,r8 r/m8  r8
        # pnX: added this missing opcode, partial coverage only yet
        elif instruction.opcode == 0x80:
            ...
            if op1.type == pydasm.OPERAND_TYPE_REGISTER:
                ...
                op2value = 0
                if op2.type == pydasm.OPERAND_TYPE_IMMEDIATE:
                    op2value = op2.immediate & self.get_mask(size)
                elif op2.type == pydasm.OPERAND_TYPE_REGISTER:
                    op2value = self.get_register(op2.reg, size)
                else:
                    raise NotImplementedError()
    ...
    def CMP(self, instruction):
        ...
        #3B /r CMP r32,r/m32 Compare r/m32 with r32
            ...
            if op2.type == pydasm.OPERAND_TYPE_REGISTER:
                ...
                # PATCHED: cmp would both take size 2 memory deref and operand, although 4 byte deref is needed...
                size = 4
                op2value = self.get_memory_address(instruction, 2, size)
    ...
    def MOV(self, instruction):
        ...
        #A3 MOV moffs32*,EAX Move EAX to (seg:offset)
            ...
            if instruction.fs_override():
                ...
                # PATCH: Isn't this basically just mov dword ptr ds:[], EAX in 32bit mode?
                # print "[!] Please add this segment"
                # return False
                self.set_memory(instruction.op1.displacement, op2value, asize)
    ...
    def XOR(self, instruction):
        ...
        #31 /r XOR r/m32,r32 r/m32  r32
            ...
            if op1.type == pydasm.OPERAND_TYPE_REGISTER:
                ...
                # pnX: fixed this, memory would be addressed with 2 instead of 4 bytes (latter is correct)
                offset_size = 4
                op1value = self.get_memory_address(instruction, 1, offset_size)
                op2value = self.get_register(op2.reg, size)
        ...
        #32 /r XOR r8,r/m8 r/m8  r8
        # pnX: added this opcode, like 30 but with opposite direction
        elif instruction.opcode == 0x32:

            size = 1

            print pydasm.OPERAND_TYPE_MEMORY
            print pydasm.OPERAND_TYPE_REGISTER
            print pydasm.OPERAND_TYPE_IMMEDIATE
            print "optypes:", op1.type, op2.type
            ...
            if op2.type == pydasm.OPERAND_TYPE_REGISTER:
                ...
                offset_size = 4
                op1value = self.get_register(op1.reg, size)
                op2value = self.get_memory_address(instruction, 2, offset_size)
                print "XOR: op1: 0x%x - op2 addr: 0x%x" % (op1value, op2value)

                # Do logic
                op2valuederef = self.get_memory(op2value, size)

                result = op2valuederef ^ op1value
                print "XOR: op1: 0x%x - op2: 0x%x (0x%x) - result: 0x%x" % (op1value, op2valuederef, op2value, result)

                self.set_flags("LOGIC", op2valuederef, op1value, result, size)
